...
// ─── Shared multipart POST: meta JSON + JPEG binary ─────────────────────────
// /frame and /batch send exactly the same body shape and differ only in the
// URL and in what the caller does with the reply, so the builder lives here
// once.  Returns the HTTP status code, or -1 if the request never completed.
static int post_multipart(const char *url, const char *meta_json,
                          ...
                          char *reply, size_t reply_cap, int *reply_len)
{
    ...
    static char head[8192];  // static: keeps this off the task stack (one task only)
    ...
    ESP_LOGI(TAG, "POST %s   img=%u total=%d", url, (unsigned)jpg_len, total);
    ...
    if (write_chunk(c, head, hl) != ESP_OK ||
        (jpg_len && write_chunk(c, (const char *)jpg_buf, jpg_len) != ESP_OK) ||
    ...
    int off = 0;
    if (reply && reply_cap) {
        while (off < (int)reply_cap - 1) {
            int r = esp_http_client_read(c, reply + off, reply_cap - 1 - off);
            if (r <= 0) break;
            off += r;
        }
        reply[off] = '\0';
    }
    if (reply_len) *reply_len = off;
                               ...
                               size_t         jpg_len)
{
    char reply[512];
    int  off  = 0;
    int  code = post_multipart(BW_UPLOAD_URL, meta_json, jpg_buf, jpg_len,
                               reply, sizeof(reply), &off);
    if (code != 200 || off == 0) {
...
// ─── /batch POST — one suppressed event ─────────────────────────────────────
// Deliberately one record per request: records are small, and each is then
// independently retryable, so a single bad record cannot strand the backlog.
// The reply is ignored beyond the status code — mode changes belong to /frame.
esp_err_t bw_http_post_batch(const char *meta_json,
                             const uint8_t *jpg_buf, size_t jpg_len)
{
    int code = post_multipart(BW_BATCH_URL, meta_json, jpg_buf, jpg_len,
                              NULL, 0, NULL);
    if (code < 200 || code >= 300) {
        ESP_LOGW(TAG, "  batch record rejected (status=%d)", code);
        return ESP_FAIL;
    }
    return ESP_OK;
}
